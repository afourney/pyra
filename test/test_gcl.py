...
class TestProcessor(unittest.TestCase):
    ...
    def _assert_access_functions(self, query, extents):
        ...
        for k in positions + positions[::-1]:
            ...
            self.assertEqual( query._first_starting_at_or_after(k),
                              starting_after[0] if starting_after else (INF,INF) )
            self.assertEqual( query._first_ending_at_or_after(k),
                              ending_after[0] if ending_after else (INF,INF) )
            self.assertEqual( query._last_ending_at_or_before(k),
                              ending_before[-1] if ending_before else (-INF,-INF) )
            self.assertEqual( query._last_starting_at_or_before(k),
                              starting_before[-1] if starting_before else (-INF,-INF) )

        self.assertEqual(list( query ), [ slice(u,v+1) for u,v in extents ])
        self.assertEqual(list( query.iterator(reverse=True) ),
                         [ slice(u,v+1) for u,v in extents[::-1] ])
    ...
    def test_and_access_functions(self):
        ...
        self._assert_access_functions( g.And(a,b),
                                       [(0,5), (2,8), (5,10), (7,13), (10,15)] )
        self._assert_access_functions( g.And(b,a),
                                       [(0,5), (2,8), (5,10), (7,13), (10,15)] )
    ...
    def test_not_containing_access_functions(self):
        ...
        self._assert_access_functions( g.NotContaining(a,g.Position(4)),
                                       [(5,10), (6,11), (7,12)] )
    ...
    def test_not_contained_in_access_functions(self):
        ...
        self._assert_access_functions( g.NotContainedIn(g.Length(1),g.Slice(slice(4,12))),
                                       [(0,0), (1,1), (2,2), (3,3),
    ...
    def test_negative_containment_queries(self):
        ...
        self._assert_access_functions( g.parse('("<scene>".."</scene>") /> "hamlet"'),
                                       [(6,8)] )
        self._assert_access_functions( g.parse('"hamlet" /< ("<title>".."</title>")'),
                                       [(4,4)] )
        self._assert_access_functions( g.parse('("<scene>".."</scene>") /> ("hamlet" + "ghost")'),
                                       [] )
    ...
    def test_negative_containment_reference(self):
        ...
        for a in lists:
            for b in lists:
                ...
                not_containing = [ (u,v) for u,v in a
                                   if not any(u <= p and q <= v for p,q in b) ]
                not_contained_in = [ (u,v) for u,v in a
                                     if not any(p <= u and v <= q for p,q in b) ]
    ...
    def test_empty_operands(self):
        ...
        g = GCL(InvertedIndex([]))
        for operator in (g.And, g.Or, g.BoundedBy, g.Containing, g.ContainedIn,
                         g.NotContaining, g.NotContainedIn):
    ...
    def test_phrase_access_functions(self):
        ...
        self._assert_access_functions( g.And(g.Phrase('a','b'),g.Term('x')),
                                       [(2,4), (6,9)] )
    ...
    def test_lazy_access_functions(self):
        ...
        self._assert_access_functions( g.And(g.Containing(a,a),g.ContainedIn(b,b)),
                                       [(0,5), (2,8), (5,10), (7,13), (10,15)] )
        self._assert_access_functions( g.BoundedBy(g.Or(a,a),g.Or(b,b)), [(0,10), (5,15)] )
        self._assert_access_functions( g.Containing(g.And(a,b),g.Or(a,b)),
                                       [(0,5), (2,8), (5,10), (7,13), (10,15)] )
        self._assert_access_functions( g.ContainedIn(g.Or(a,b),g.And(a,b)),
                                       [(0,3), (2,5), (5,8), (7,10), (10,13), (12,15)] )
        self._assert_access_functions( g.NotContaining(a,g.NotContainedIn(b,a)),
                                       [(0,3), (5,8), (10,13)] )
        self._assert_access_functions( g.NotContainedIn(b,g.NotContaining(a,b)),
                                       [(2,5), (7,10), (12,15)] )
